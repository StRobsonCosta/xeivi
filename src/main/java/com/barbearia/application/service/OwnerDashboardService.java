...
public class OwnerDashboardService {
  ...
  public OwnerDashboardDto getOwnerDashboard(LocalDate startDate, LocalDate endDate) {
    ...
    double ownerProfit = appointments.stream().mapToDouble(Appointment::getOwnerRevenue).sum();

    double totalCosts =
        materialCostRepository.findByCostDateBetween(startDate, endDate).stream()
            .mapToDouble(cost -> cost.getAmount())
            .sum();

    List<DailyMovement> movements =
        appointments.stream()
                ...
                entry ->
                    new DailyMovement(
                        ...
                        entry.getValue().stream()
                            .mapToDouble(a -> a.getServiceOffer().getPrice())
                            .sum()))
    ...
    List<DailyMovement> slowestDays =
        movements.stream()
